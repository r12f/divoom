...
make-symbols:
    if ("{{BUILD_OS}}" -eq "windows") { \
        ...
        $objcopyPath = if ("{{BUILD_IS_CROSS_COMPILE}}" -eq "true") { "{{BUILD_TOOL_TARGET}}-objcopy"; } else { "objcopy" }; \
        $stripPath = if ("{{BUILD_IS_CROSS_COMPILE}}" -eq "true") { "{{BUILD_TOOL_TARGET}}-strip"; } else { "strip" }; \
