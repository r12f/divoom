...
BUILD_IS_CROSS_COMPILE := env_var_or_default("BUILD_IS_CROSS_COMPILE", "")
...
make-symbols:
    if ("{{BUILD_OS}}" -eq "windows") { \
        ...
        $objcopyPath = if ("{{BUILD_IS_CROSS_COMPILE}}" -eq "true") { "{{BUILD_TARGET}}-objcopy"; } else { "objcopy" }; \
        $stripPath = if ("{{BUILD_IS_CROSS_COMPILE}}" -eq "true") { "{{BUILD_TARGET}}-strip"; } else { "strip" }; \
...
_make-symbols OBJCOPY_PATH="objcopy" STRIP_PATH="strip" FILE_NAME="divoom-cli":
    ...
    Remove-Item -Path "{{BUILD_OUTPUT_FOLDER}}/{{FILE_NAME}}.debug";
